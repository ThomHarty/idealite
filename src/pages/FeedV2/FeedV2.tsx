...
export const FeedV2 = ({navigation}: any) => {
  ...
  const userCards: ISwipeCardsProps['cards'] = [1, 2, 3, 4, 5, 6, 7].map(
    (_, i) => {
      return {
        id: i + 10,
        cardType: 'user',
  ...
  const onPress: ISwipeCardsProps['onPress'] = id =>
    navigation.navigate('FeedUserProfile');
  const onSwipedRight: ISwipeCardsProps['onSwipedRight'] = id =>
    setSwipedRightIds(current => [...current, id]);
  const onSwipedLeft: ISwipeCardsProps['onSwipedLeft'] = id =>
    setSwipedLeftIds(current => [...current, id]);
  const topRightIconOnPress: ISwipeCardsProps['topRightIconOnPress'] = () =>
    navigation.navigate('FeedFilters');
  const topLeftIconOnPress: ISwipeCardsProps['topLeftIconOnPress'] = id =>
    console.log('left icon pressed: ', id);
