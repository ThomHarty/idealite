...
export const useDisabledColors = ({variant, disabled}: Props) => {
  ...
  const button = useAnimatedStyle(() => {
    return {
      backgroundColor: interpolateColor(
        shared.value,
        [0, 1],
        [styles.button.backgroundColor, disabledStyles.button.backgroundColor],
      ),
      borderColor: interpolateColor(
        shared.value,
        [0, 1],
        [styles.button.borderColor, disabledStyles.button.borderColor],
      ),
    };
  });
