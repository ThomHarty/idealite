...
export const usePressedColors = ({variant, pressed}: Props) => {
  ...
  const button = useAnimatedStyle(() => {
    return {
      backgroundColor: interpolateColor(
        shared.value,
        [0, 1],
        [styles.button.backgroundColor, pressedStyles.button.backgroundColor],
      ),
      borderColor: interpolateColor(
        shared.value,
        [0, 1],
        [styles.button.borderColor, pressedStyles.button.borderColor],
      ),
    };
  });
