...
export const Dropdown = memo(
  ({
    ...
    const {layout} = useLayoutStyles({
      isOpen,
      hasSelected: selected ? true : false,
    });
    ...
    return (
      <>
        ...
        <Pressable onPress={handleDropdown} testID="open-dropdown">
          <Animated.View
            style={[baseStyles.selectedContainer, disabledColors.input]}>
            <Text style={baseStyles.selectedText}>{selectedItem?.key}</Text>
          </Animated.View>
        ...
        {showDropdown && (
          <View style={baseStyles.dropdownContainer}>
            <Animated.View
              style={[baseStyles.dropdownWrapper, layout.dropdown]}>
              {/* selected value */}
              <Animated.View
                style={[
                  ...
                  renderItem={({item}) => (
                    <Pressable onPress={() => itemOnPress(item)}>
                      <View style={baseStyles.itemContainer}>
                        <Text style={baseStyles.itemText}>{item.value}</Text>
                      </View>
                    </Pressable>
                  )}
