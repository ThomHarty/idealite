  ...
  ({
    ...
    const gesture = Gesture.Pan()
      ...
      .onEnd(({velocityX}) => {
        ...
        if (dest > 0) {
          runOnJS(onSwipedRight)(cards[activeIndex].id);
        }
        if (dest < 0) {
          runOnJS(onSwipedLeft)(cards[activeIndex].id);
        }
        if (dest === 0) {
    ...
    const getStyles = (index: number) => {
      ...
      if (activeIndex < index) {
        styles = getChildCardStyle(index);
      }
    ...
    useEffect(() => {
      if (activeIndex < cardsLength - 4) {
        if (swipeRight) {
          _handleButtonSwipeRight();
        }
        if (swipeLeft) {
          _handleButtonSwipeLeft();
        }
      }
    ...
    return (
      <>
        <Pressable
          style={[baseStyles.container, disabled && {opacity: 0}]}
                  ...
                  return (
                    <Animated.View
                      ...
                      <SwipeIcons
                        leftOnPress={
                          topLeftIconOnPress
                            ? () => topLeftIconOnPress(card.id)
                            : undefined
                        }
                        rightOnPress={
                          topRightIconOnPress
                            ? () => topRightIconOnPress(card.id)
                            : undefined
                        }
                        ...
                        crossStyle={
                          activeIndex === i ? crossStyle : {opacity: 0}
                        }
                      />
                      {card.cardType === 'user' && (
            ...
            <View style={baseStyles.cardsContainer}>
              ...
              <View style={baseStyles.card}>
                <SwipeIcons
                  leftOnPress={
                    topLeftIconOnPress
                      ? () => topLeftIconOnPress(cards[nextIndex].id)
                      : undefined
                  }
                  rightOnPress={
                    topRightIconOnPress
                      ? () => topRightIconOnPress(cards[nextIndex].id)
                      : undefined
                  }
