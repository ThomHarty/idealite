  ...
  return (
    <>
      <Pressable
        ...
        onPress={!disabledEdit && onPress ? () => onPress(id) : undefined}>
        <View>
          ...
          {details && (
            <View style={baseStyles.detailsContainer}>
              <Typography text={details} size="tiny" color={COLORS.GREY} />
            </View>
          )}
        </View>
        {!disabledEdit && (
          <Icon name="edit" testID="icon" iconStyles={baseStyles.icon} />
        )}
      </Pressable>
      <Divider />
    </>
