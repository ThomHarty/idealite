...
export const CodeInput = memo(
  ({error, correct = false, onEndCodeInput}: ICodeInputProps): ReactElement => {
    ...
    const {successColors} = useSuccessColors({
      success: code[5] !== '' && correct,
    });
    const {invalidColors} = useInvalidColors({
      invalid: code[5] !== '' && hasError,
    });
    // hooks
